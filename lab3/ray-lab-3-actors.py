...
# Create an instance of the MethodStateCounter actor
counter = MethodStateCounter.remote()

# Invoke the actor multiple times
for i in range(10):
    invoker_name = random.choice(CALLERS)
    ray.get(counter.invoke.remote(invoker_name))

# Get number of times an invoker name was called
for invoker_name in CALLERS:
    count = ray.get(counter.get_invoker_count.remote(invoker_name))
    print(f"Invoker {invoker_name} was called {count} times")

# Get a list of values computed by invoker name
for invoker_name in CALLERS:
    values = ray.get(counter.get_invoker_values.remote(invoker_name))
    print(f"Values computed by {invoker_name}: {values}")

# Get state of all invokers
all_invokers = ray.get(counter.get_all_invoker_state.remote())
print(f"All invokers state: {all_invokers}")
...
class PiEstimator:
    ...
    def update(self, points_inside_circle, total_points):
...
# Define a task to compute candidates for pi
@ray.remote
def compute_points_inside_circle(num_points):
...
# Initialize the PiEstimator actor
pi_estimator = PiEstimator.remote()

# Set the number of tasks and points per task
num_tasks = 100
num_points_per_task = 1000

# Compute candidates for pi in parallel and update the PiEstimator actor
for _ in range(num_tasks):
    points_inside_circle = compute_points_inside_circle.remote(num_points_per_task)
    pi_estimator.update.remote(points_inside_circle, num_points_per_task)

# Get the final pi estimate
pi_estimate = ray.get(pi_estimator.get_pi_estimate.remote())
