...
def process_data(data):
    # This is a simple example, you can replace this with your desired operation
    return sum(data)
...
# Put the large list and dictionary into the object store
large_list_ref = ray.put(large_list)
large_dict_values_ref = ray.put(list(large_dict.values()))

# Call the remote function on the large list and dictionary
result_list_ref = process_data.remote(large_list_ref)
result_dict_ref = process_data.remote(large_dict_values_ref)

# Get the results
result_list = ray.get(result_list_ref)
result_dict = ray.get(result_dict_ref)

print(f"Result for large list: {result_list}")
print(f"Result for large dictionary: {result_dict}")
